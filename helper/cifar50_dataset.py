...
def _prepare_cifar50f_data():
    data_path = '/home/huwenp/Dataset/CIFAR50/features'
    # url = 'http://www.cs.toronto.edu/~kriz/cifar-10-python.tar.gz'
    # file_manager.create_dirname_if_not_exist(data_path)
    # file_name = os.path.basename(url)
    # full_path = os.path.join(data_path, file_name)
    folder = os.path.join(data_path, 'checkpoint0')
    train_x = []
    train_y = []
    for i in range(0, 3):
        file_path = os.path.join(folder, 'traindata_batch_{0:d}.pt'.format(i))
...
class Cifar50Dataset(cl_dataset.CLClassDataSet):

    def __init__(self, *args, **kwargs):
        self._train_x, self._train_y, self._test_x, self._test_y = \
            _prepare_cifar50f_data()
        super(Cifar50Dataset, self).__init__(*args, **kwargs)
        self.data_size = 0
